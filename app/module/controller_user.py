...
def proses_deteksi_koreksi():
    if request.method == "POST":
        ...
        # preprocessing
        token_kata = preprocessing(jawaban)
        ...
        # return jsonify(len(result), result, len(unigram), unigram, len(bigram_kiri), bigram_kiri,len(bigram_kanan), bigram_kanan,len(trigram), trigram)
        ...
        # return jsonify(len(tot_unigram), tot_unigram, len(tot_bigram_kiri), tot_bigram_kiri,len(tot_bigram_kanan), tot_bigram_kanan,len(tot_trigram), tot_trigram)
        # hitung probabilitas
        ...
        #         print(number)
                ...
                # # kata = j[1]
                # print(j[0])
        # rank = sorted(skor_kata, key=itemgetter(1), reverse=True)
...
def proses_coba_penilaian():
    if request.method == "POST":
        ...
        # menambah kata di list stopword
        ...
        for i in kunci:
            if i not in stopwords:
                prepro_kunci.append(i)

        prepro_jawaban = []
        for i in jawaban:
            if i not in stopwords:
                prepro_jawaban.append(i)
